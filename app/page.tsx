      ...
      <header className="fixed inset-x-0 top-0 z-50 border-b border-white/[0.06] bg-[#090b0f]/75 backdrop-blur-2xl">
        <div className="mx-auto flex h-[74px] max-w-7xl items-center justify-between px-6">
          ...
          <nav className="hidden items-center gap-9 text-[13px] text-white/45 md:flex">
            ...
            {/* TOOLS DROPDOWN — matches the Apps menu */}
            <div className="group relative">
              <button
                type="button"
                className="flex h-[74px] items-center gap-2 transition hover:text-white focus:text-white focus:outline-none"
              ...
              >
                Tools
                <svg viewBox="0 0 12 8" fill="none" className="h-2 w-3 transition-transform duration-200 group-hover:rotate-180 group-focus-within:rotate-180" aria-hidden="true">
                  <path d="M1 1.5L6 6.5L11 1.5" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </button>
              <div className="absolute left-1/2 top-[64px] h-5 w-[330px] -translate-x-1/2" />
              <div className="invisible absolute left-1/2 top-[68px] w-[330px] -translate-x-1/2 translate-y-2 rounded-[24px] border border-white/[0.09] bg-[#0d1015]/95 p-2.5 opacity-0 shadow-[0_30px_80px_rgba(0,0,0,0.55)] backdrop-blur-2xl transition-all duration-200 group-hover:visible group-hover:translate-y-0 group-hover:opacity-100 group-focus-within:visible group-focus-within:translate-y-0 group-focus-within:opacity-100">
                <div className="px-3 pb-2 pt-2">
                  <p className="text-[9px] font-semibold uppercase tracking-[0.2em] text-white/20">Bodolvo Tools</p>
                </div>
                <div className="max-h-[min(65vh,500px)] overflow-y-auto overscroll-contain">
                  {toolCatalog.map((tool) => (
                    <Link key={tool.slug} href={`/apps/${tool.slug}`} className="group/tool flex items-center gap-4 rounded-[18px] border border-transparent p-3.5 transition duration-200 hover:border-white/[0.07] hover:bg-white/[0.045]">
                      <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-[14px] border border-violet-200/[0.12] bg-violet-400/[0.06] text-sm font-semibold text-violet-100/75">
                        {tool.name[0]}
                      </div>
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center justify-between gap-4">
                          <p className="text-[13px] font-medium text-white/85">{tool.name}</p>
                          <span className="text-sm text-white/18 transition duration-200 group-hover/tool:translate-x-1 group-hover/tool:text-white/55">→</span>
                        </div>
                        <p className="mt-1 text-[10px] leading-4 text-white/30">{tool.description}</p>
                      </div>
                    </Link>
                  ))}
                </div>
                <div className="mt-1 border-t border-white/[0.05] px-3 py-2">
                  <Link href="/tools" className="block text-[11px] text-white/50 transition hover:text-white">View all tools →</Link>
                </div>
              </div>
            </div>
